...
export default function EditSkillsSection({
  ...
  const handleUpdateSkillItems = async (e: FormData, id: number) => {
    try {
  ...
  const handleAddSkill = async () => {
    try {
      await createSkillItem({ title: "Neue Karte", desc: "" });
      const newSkillItems = await getSkills();
      setSkillItems(newSkillItems?.skillItems);
    } catch (error) {
      console.error(error);
    ...
    return (
      ...
      >
        <h2 className="text-xl mb-4 font-bold">
          Informationskarten-Sektion aktualisieren ({skillItems.length}/5)
        </h2>
        <div className="flex gap-4 flex-nowrap overflow-x-auto pb-3">
          {skillItems?.map((item) => (
            <form
              key={item.id}
              action={(e) => handleUpdateSkillItems(e, item.id)}
